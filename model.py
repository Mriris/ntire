...
class Dehaze(nn.Module):
    ...
    def forward(self, input):
        x, x_layer1, x_layer2 = self.encoder(input) # x_layer1 (1,256,120,160) x_layer2 (1,512,60,80)

        x_mid = self.mid_conv(x)

        x = self.up_block1(x_mid)
        x = self.attention1(x) # (1,256,60,80)

        x = torch.cat((x, x_layer2), 1) # (1,768,60,80)
        x = self.up_block1(x)
        x = self.attention2(x) # (1,192,120,160)

        x = torch.cat((x, x_layer1), 1) # (1,448,120,160)
        x = self.up_block1(x)
        x = self.up_block1(x) # (1,28,480,640)

        dout2 = self.enhancer(x) # (1,28,480,640)
