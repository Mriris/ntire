...
try:
    MyEnsembleNet.load_state_dict(torch.load('weight/best.pkl'))
    print('--- weight loaded ---')
except:
...
with torch.no_grad():
    ...
    for batch_idx, (hazy, name, orig_size) in enumerate(val_loader):
        ...
        imwrite(img_list[batch_idx], os.path.join(imsave_dir, str(name)[2:-3]))
    time_cost = float(sum(time_list) / len(time_list))
    
    print('{} images, running time per image: {}'.format(len(time_list), time_cost))
